...
- **feat(compress): 服务端强制压缩 — 模型忽略 nudge 时自动压缩 (closes #269)** — nudge 只是建议，模型不调 `compress` 就不压缩；长 agentic 任务可连续 20+ 轮 OVER-LIMIT（77%→87%）模型始终不压缩，无服务端兜底。新增 `src/auto-compress.ts`：在 `context` transform 的 processTurn 之后，若 (a) 连续 `afterIgnores`（默认 3）轮 over-limit 且模型未压缩（streak），或 (b) usage ≥ `hardThreshold`（默认 95%），则服务端自动压缩最大的可压缩范围——`pickAutoRanges` 贪心选 viable 范围直到 projected usage < `targetPct`（默认 80%），上限 `maxRanges`（默认 5）个——写入诚实的机械摘要（块可 `decompress` 完整还原），再重跑 processTurn 得到压缩后视图。成功的模型 `compress` 重置 streak；`usage <= targetPct` 不触发；每 turnKey 至多一次（`lastAutoCompressedTurnKey` 防同轮重复）。`compress.autoCompress` 默认 `true`，可设 `false` 或对象调参。12 个新测试（单测 + e2e：streak 触发 / hard 触发 / 禁用）
