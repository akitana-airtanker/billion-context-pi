...
- **feat(compress): 服务端强制压缩 — 模型忽略 nudge 时自动压缩 (closes #269)** — nudge 只是建议，模型不调 `compress` 就不压缩；长 agentic 任务可连续 20+ 轮 OVER-LIMIT（77%→87%）模型始终不压缩，无服务端兜底。新增 `src/auto-compress.ts`：在 `context` transform 的 processTurn 之后，若 (a) 连续 `afterIgnores`（默认 3）轮 over-limit 且模型未压缩（streak），或 (b) usage ≥ `hardThreshold`（默认 95%），则服务端自动压缩最大的可压缩范围——`pickAutoRanges` 贪心选 viable 范围直到 projected usage < `targetPct`（默认 80%），上限 `maxRanges`（默认 5）个——写入中性机械摘要（与模型驱动压缩不可区分，块可 `decompress` 完整还原），再重跑 processTurn 得到压缩后视图。护栏防深度压缩区间（剩余可压缩内容很少、每次触发的块开销可能超过收益）适得其反：净收益下限 `minEnforceNetGain`（默认 5000，扣除摘要开销后不足则跳过）、每范围仅一次、会话预算 `enforceBudget`（默认 5 次）、冷却 `cooldownGrowth × nudge.minGrowthFloor`（默认 5×，硬阈值绕过）、模型成功 `compress` 时当轮跳过（不覆盖模型自身压缩）、对模型不可见（仅宿主 UI 通知，模型不会 decompress/recompress 反应）。`usage <= targetPct` 不触发。`compress.autoCompress` 默认 `true`，可设 `false` 或对象调参。新测试覆盖单测 + e2e（streak / hard / 禁用 / 净收益下限 / 预算 / 冷却）
