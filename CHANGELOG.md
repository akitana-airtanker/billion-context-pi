...
- **fix(floor): provider-usage floor 跳过压缩后一轮的 stale anchor（PR #258 评审）** — pi 的 `getContextUsage()` 锚定最后一条带有效 usage 的 assistant 消息（跳过 aborted/error/全零）；成功 compress 落在锚点之后，下一个 LLM 调用若仍以旧锚点 floor，会在刚压缩完的上下文上重跑 emergency（nudge 注入 + 工具结果机械截断），面板也继续显示压缩前的大数字。新增 `src/floor-stale.ts`：`usageAnchorPredatesCompression(entries)` 扫描分支条目，锚点早于最后一条成功 compress toolResult（panel >0 blocks，失败/no-op 不算）时跳过 floor，context transform / `acp_status` / `/acp` 面板三处一致；下一条 assistant usage 到达即自愈。对齐 pi 自身 compaction 的 "usage source must be post-compaction" 检查。同时修正 `src/index.ts` floor 注释（原声称 "can't cause a false emergency"，对 usage-less provider 的 tree-sum 回退不成立，现为已接受取舍）
- docs(token-calibration-plan): 顶部状态横幅标注 density 校准系统已随 #258 移除，本文降级为历史设计记录；记录现行方案（发送视图估算 + 真实 usage floor + stale-anchor 守卫）与两项已接受取舍（omp #18 usage-less provider 树求和回退；显示层 CJK 未校准）
