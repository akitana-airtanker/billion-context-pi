...
test("decompress inline:true falls back to file mode when content exceeds decompressInlineMaxChars", async () => {
  const { api, handlers } = captureApi();
  // Low cap (1000 chars) so the test content (~5200 chars) triggers the fallback.
  createAcpExtension({ modelContextLimit: 200_000, decompressInlineMaxChars: 1000 } as any)(api as any);

  const stateFile = "/tmp/pai-acp-decompress-inline-cap.session.json";
  await cleanState(stateFile);
  const longText = "This is a detailed message that needs to be compressed. ".repeat(130);
  const filler = (n: string) => `filler ${n} `.repeat(400);
  const entries = [
  ...
  await compressTool.execute(
    "tc1",
    { content: [{ startId: "m00001", endId: "m00001", summary: "Detailed initial context message for the inline-cap test." }] },
    undefined, undefined, ctx,
  );
  ...
  // The content (~5200 chars) exceeds the cap (1000 chars), so it must fall back to file mode.
  assert.match(text, /exceeds the inline cap/, "result reports the inline cap was hit");
  ...
  // Crucially: the full long content (~5200 chars) is NOT in the tool result
  // (it's in the file). The result carries only a short head preview (600 chars)
  // + boilerplate, so it must be far smaller than the restored content.
  assert.ok(text.length < 2000,
    `inline content must NOT be the full restored text (result was ${text.length} chars)`);
});

test("decompress inline:true stays inline when content is under decompressInlineMaxChars", async () => {
  const { api, handlers } = captureApi();
  // High cap (100K chars) so the test content (~5200 chars) stays inline.
  createAcpExtension({ modelContextLimit: 200_000, decompressInlineMaxChars: 100_000 } as any)(api as any);

  const stateFile = "/tmp/pai-acp-decompress-inline-under-cap.session.json";
  await cleanState(stateFile);
  const longText = "This is a detailed message that needs to be compressed. ".repeat(130);
  const filler = (n: string) => `filler ${n} `.repeat(400);
  const entries = [
  ...
  await compressTool.execute(
    "tc1",
    { content: [{ startId: "m00001", endId: "m00001", summary: "Detailed initial context message for the under-cap test." }] },
    undefined, undefined, ctx,
  );
  ...
  // The content (~5200 chars) is under the cap (100K chars), so it stays inline.
...
test("decompress inline:true with decompressInlineMaxChars=0 disables the cap (unlimited inline)", async () => {
  const { api, handlers } = captureApi();
  // Cap disabled (0) so even large content stays inline.
  createAcpExtension({ modelContextLimit: 200_000, decompressInlineMaxChars: 0 } as any)(api as any);

  const stateFile = "/tmp/pai-acp-decompress-inline-no-cap.session.json";
  await cleanState(stateFile);
  // Large content (~52K chars) that would exceed the default 100K cap if it were
  // larger, but with cap=0 it stays inline regardless of size.
  const longText = "This is a detailed message that needs to be compressed. ".repeat(1300);
  const filler = (n: string) => `filler ${n} `.repeat(400);
  const entries = [
    userMsg("e1", longText),
    userMsg("e2", filler("two")), userMsg("e3", filler("three")),
  ...
  await compressTool.execute(
    "tc1",
    { content: [{ startId: "m00001", endId: "m00001", summary: "Detailed initial context message for the no-cap test." }] },
    undefined, undefined, ctx,
  );
  ...
  // Cap disabled (0), so the content stays inline even though it's large.
  assert.match(text, /inline:/, "result signals inline mode (cap disabled)");
  assert.ok(text.includes("This is a detailed message that needs to be compressed."),
    "full restored content present in the tool result (cap disabled)");
});
